function Write-SettingsFile {
    ...
    $Output += "OutputPath:$($Script:GUIActions.OutputPath)"
    ...
    foreach ($Partition in $AmigaPartitions) {
        ...
        if ($Partition.Value.ImportedFiles){
            foreach ($ImportedFile in $Partition.Value.ImportedFiles){
                $Output += "$($Partition.Name);ImportedFiles.FullPath;$($ImportedFile.FullPath);Size;$($ImportedFile.Size);CreationTime;$($ImportedFile.CreationTime);Source;$($ImportedFile.Source);$($ImportedFile.PathHeader)"  
            }
        }
        #$Output += "$($Partition.Name);ImportedFiles;$($Partition.Value.ImportedFiles)"  
...
$WPF_DP_Partition_ID76_1_AmigaDisk_Partition_2.ImportedFiles