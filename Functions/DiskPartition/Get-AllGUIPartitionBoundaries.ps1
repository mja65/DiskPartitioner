function Get-AllGUIPartitionBoundaries {
    ...
    for ($i = 0; $i -lt $AllPartitionBoundaries_MBR.Count; $i++) {
        ...
        if (($i+1) -lt $AllPartitionBoundaries_MBR.Count){
            ...
            $AllPartitionBoundaries_MBR[$i].BytesAvailableRight =  $WPF_DP_Disk_MBR.DiskSizeBytes - $WPF_DP_Disk_MBR.MBROverheadBytes - $AllPartitionBoundaries_MBR[$i].EndingPositionBytes
        }                  
     }
