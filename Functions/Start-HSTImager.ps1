function Start-HSTImager {
    ...
    if ($Command -eq 'Blank'){
        ...
        & $Script:Settings.HSTLocation blank $DestinationPath $ImageSize >$Logoutput            
    }
    elseif ($Command -eq 'rdb init'){
        Write-InformationMessage -Message 'Initialising partition'
        & $Script:Settings.HSTLocation rdb init $DestinationPath $Options >$Logoutput            
    }
    elseif ($Command -eq 'rdb filesystem add'){
        Write-InformationMessage -Message ('Adding Filesystem '+$DosType+' to RDB')
        & $Script:Settings.HSTLocation rdb filesystem add $DestinationPath $FileSystemPath $DosType $Options >$Logoutput            
    }
    elseif ($Command -eq 'rdb part add'){
        ...
        & $Script:Settings.HSTLocation rdb part add $DestinationPath $DeviceName $DosType $SizeofPartition $Options --mask 0x7ffffffe --buffers 300 --max-transfer 0xffffff >$Logoutput
    }
    elseif ($Command -eq 'rdb part format'){
        Write-InformationMessage -Message ('Formatting partition '+$VolumeName)
        & $Script:Settings.HSTLocation rdb part format $DestinationPath $PartitionNumber $VolumeName >$Logoutput         
    }   
    elseif ($Command -eq 'fs extract') {
        ...
        & $Script:Settings.HSTLocation fs extract $SourcePath $DestinationPath $Options >$Logoutput                                
    }
    elseif ($Command -eq 'fs copy') {
        Write-InformationMessage -Message ('Writing file(s) to HDF image for: '+$SourcePath+' to '+$DestinationPath) 
        & $Script:Settings.HSTLocation fs copy $SourcePath $DestinationPath $Options >$Logoutput  
    } 
    ...
    foreach ($ErrorLine in $CheckforError){
        if ($ErrorLine -match " ERR]"){
            ...
            Copy-Item -Path $Logoutput -Destination ($Script:Settings.LogFolder+$Script:Settings.LogDateTime+'_LastHSTErrorLogFull.txt')        
        }
    }
    if ($ErrorCount -ge 1){       
        return $false
